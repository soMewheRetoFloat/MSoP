...
# save_path = os.path.join(out_dir, 'samples.txt')
...
save_file = open(output_data, 'w', encoding='utf-8')

# encode the beginning of the prompt
with open(input_data, 'r', encoding='utf-8') as f:
    starts = [line.split('"')[3] for line in f.readlines()]
    ...
    with torch.no_grad():
        with ctx:
            for k in range(num_samples):
                ...
                output.replace(' ', '')
                print(output)
                save_file.write('{' + f'"question": "{start}", "answer": "{output}"' + '}\n')
                print('---------------')
save_file.close()
